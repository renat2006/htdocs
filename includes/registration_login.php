<?php
	...
	if (isset($_POST['reg_user'])) {
		...
		if (empty($username)) {  array_push($errors, "Имя пользоватнля обязательно"); }
		if (empty($email)) { array_push($errors, "Email обязателен"); }
		if (empty($password_1)) { array_push($errors, "Кажется пароль был введён неверно"); }
		...
		if ($user) { // if user exists
			if ($user['username'] === $username) {
			  array_push($errors, "Это имя пользователя занято");
			}
			if ($user['email'] === $email) {
	...
	if (isset($_POST['login_btn'])) {
		...
		if (empty($errors)) {
			...
			if (mysqli_num_rows($result) > 0) {
				...
				array_push($errors, 'Проверьте введёные данные');
			}
		}
	}
